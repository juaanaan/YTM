...
def generate_event_summary(code, flight_number, pickup_address, dropoff_address, pickup_time, client_name, email, phone, passengerCount, luggageCount, driverCode, pickup_maps_link, addOns):
...
def create_event(service, calendar_id, code, flight_number, pickup_address, dropoff_address, pickup_time, client_name, email, phone, passengerCount, luggageCount, pickup_maps_link, price, whatsapp_link, color_id, addOns):
...
def write_to_google_sheets(service, spreadsheet_id, data):
    ...
    for row in data:
        ...
        code, pickup_time, entry_or_exit, pickup_time_hour, pickup_address, dropoff_address, price = row
        ...
        if match:
            ...
            # Agregamos los datos a la lista de valores
            values.append([code, pickup_time, entry_or_exit, pickup_time_hour, extracted_address, price])
        else:
            print("No se encontró un código postal que comience con '07' o la dirección no sigue el formato esperado.")
            values.append([code, pickup_time, entry_or_exit, pickup_time_hour, non_airport_address, price])
...
def main():
    ...
    url = f"https://warpdrive.staging.transferz.com/transfercompanies/journeys?excludedStatuses=COMPLETED&excludedStatuses=CANCELLED_FREE&excludedStatuses=CANCELLED_WITH_COSTS&excludedStatuses=BOOKER_CANCELLED&excludedStatuses=SUPPLIER_CANCELLED&createdAfter={formatted_time}&page=0&size=20"
    ...
    range_name = 'Hoja 1!A2:A'  # Ajusta el rango según tu hoja de cálculo
    ...
    # Realizar la solicitud HTTP para obtener datos de transferencias
    ...
    if response.status_code == 200:
        ...
        if results:
            for result in results: # Obtención de datos para transfer
                ...
                event_summary = generate_event_summary(code, flight_number, pickup_address, dropoff_address, result.get("pickupTime", {}).get("localTime"), client_name, email, phone, passengerCount, luggageCount, driverCode, pickup_maps_link, addOns)
                whatsapp_link = generate_whatsapp_link(event_summary)

                # Creación del nuevo evento

                create_event(service, 'juanangonzcomas@gmail.com', code, flight_number, pickup_address, dropoff_address, result.get("pickupTime", {}).get("localTime"), client_name, email, phone, passengerCount, luggageCount, pickup_maps_link, price, whatsapp_link, color_id, addOns)
                ...
                rows_to_append.append([code, pickup_day, entry_or_exit, pickup_hour, pickup_address, dropoff_address, price])
